...
class bmd_reconcil(models.Model):
    ...
    def _default_profile(self):
        company_id = self.env['res.company']._company_default_get("bmd.reconcil.profile")
        return self.env["bmd.reconcil.profile"].search([("company_id","=",company_id)], limit=1)
    
    task_id = fields.Many2one(
        "automation.task", "Task", required=True, index=True, ondelete="cascade"
    )
    profile_id = fields.Many2one("bmd.reconcil.profile", "Profil", required=True,
                                 ondelete="restrict",
                                 default=_default_profile)
     
    reconciled = fields.Integer("Reconciled", compute="_compute_reconciled", store=False)
    ...
    def _run(self, taskc):
        ...
        for inv in invoices:
            ...
            if not inv.account_id.reconcile:
                taskc.logw("Setze Konto ausgleichbar", ref="account.account,%s" % inv.account_id.id)
            ...
            try:
                ...
                taskc.loge("Fehler beim Ausgleich der Rechnung", ref="account.invoice,%s" % inv.id)
                raise e
