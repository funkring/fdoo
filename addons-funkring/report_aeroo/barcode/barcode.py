...
def make_barcode(code, code_type='ean13', rotate=None, height=50, xw=1, fontsize=12):
    if code:
        if code_type.lower()=='ean13':
            bar=EanBarCode()            
            im = bar.getImage(code,height=height,xw=xw, rotate=rotate, fontsize=fontsize)
        elif code_type.lower()=='code128':           
            im = get_code(code, xw=xw, h=height,rotate=None)
