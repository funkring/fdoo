...
class Parser(report_sxw.rml_parse):
    ...
    # address_text(o.dest_picking_id and o.dest_picking_id.address_id or None)
    ...
    def _group_by_address(self):
        ...
        for line in self.objects:
            address = addresses.get(line.delivery_address_id.id)
            if not address:
                lines = [line]
                delivery_code = time.strftime("%Y%m%d")
                if line.delivery_address_id:
                    delivery_code+= str(line.delivery_address_id.id)
                address = { "address" : line.delivery_address_id, 
                            "delivery_code" : delivery_code,
                            "client_order_ref" : line.sale_order_id and line.sale_order_id.client_order_ref or "",
                            "lines" : lines
                }
                addresses[line.delivery_address_id.id]=address
                res.append(address)
            else:
