...
class hotel_room_type(osv.osv):
    ...
    _columns = { 
        'cat_id':fields.many2one('product.category','category',required=True,ondelete="cascade",select=True),
...
class hotel_folio(osv.osv):
    ...
    def _invoiced(self, cursor, user, ids, name, arg, context=None):
        return  self.pool.get('sale.order')._invoiced(cursor, user, ids, name, arg, context=context)
    ...
    _columns={
          ...
          'room_lines': fields.one2many('hotel_folio.line','folio_id',string="Room Lines"),
          'service_lines': fields.one2many('hotel_service.line','folio_id',string="Service Lines"),
    }
    ...
    def action_cancel(self, cr, uid, ids, context={}):
        res = self.pool.get('sale.order').action_cancel(cr, uid, ids, context={})        
        if ids:
...
class hotel_folio_line(osv.osv):
    ...
    def button_done(self, cr, uid, ids, context={}):
        ...
        wf_service = netsvc.LocalService("workflow")
        res = self.write(cr, uid, ids, {'state':'done'}) and res
        for line in self.browse(cr,uid,ids,context):
...
class hotel_service_type(osv.osv):
    ...
    _columns = { 
        'ser_id':fields.many2one('product.category','category',required=True,ondelete="cascade",select=True),
