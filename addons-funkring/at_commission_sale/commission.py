...
class commission_line(osv.osv):
    ...
    def _update_bonus(self, cr, uid, salesman_ids, period_ids, context=None):
        ...
        cr.execute("SELECT s.id team_id, cl.period_id, cl.partner_id, SUM(cl.price_sub) "
                   " FROM at_commission_line cl "
                   " INNER JOIN res_users u ON u.id = cl.salesman_id "
                   " INNER JOIN crm_case_section s ON s.id = u.context_section_id "
    ...
    _columns = {
        ...
        "sales_bonus_line_id" : fields.many2one("at_commission_sale.bonus_line","Sales Bonus",ondelete="restrict")
    }    
    _inherit = "at_commission.line"
commission_line()
