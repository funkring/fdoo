    ...
    def timesheet_lines(self, timesheet):
        ...
        while date_cur <= date_to:
            ...
            if value:
                ...
                attendance_lines = value.get("attendances",None)
                if attendance_lines:
                    for attendance_line in attendance_lines:
                        time_from = attendance_line.get("from")
                        time_to = attendance_line.get("to")
                        if time_from and time_to:
                            time_to_date = time_to.split(" ")[1]
                            if time_to_date != "24:00:00":
                                line.attendance.append(Attendance(util.timeToStrHours(time_from),util.timeToStrHours(time_to)))
