...
def _jdbc_connect_jpype(jclassname, jars, libs, *driver_args):
    ...
    if not jpype.isJVMStarted():
        ...
        args.append('-Dfile.encoding=ISO-8859-1')
...
#   optional callproc(self, procname, *parameters) unsupported
    ...
    def execute(self, operation, parameters=None, maxrows=0):
        if not parameters:
            parameters = ()
        self._close_last()
...
def _to_time(java_val):
    d = datetime.datetime.strptime(str(java_val), "%H:%M:%S")
