...
def render_bootstrap_template(template, values=None, debug=False, db=None, **kw):
    ...
    values['disable_database_manager']=openerp.tools.config['disable_database_manager']
    try:
...
class Database(http.Controller):
    ...
    def selector(self, **kw):
        ...
        return env.get_template("database_selector.html").render({
            'databases': dbs,
            'debug': request.debug,
            'disable_database_manager' : openerp.tools.config['disable_database_manager']
        })
...
class Binary(http.Controller):
    ...
    def saveas_ajax(self, data, token):
        ...
        if filename_field:
            fields.append(filename_field)
        if data:
            res = len(fields) > 1 and (id and Model.read([int(id)], fields[1:], context)[0] or Model.default_get(fields[1:], context)) or {}
            res[field]=data
        elif id:
