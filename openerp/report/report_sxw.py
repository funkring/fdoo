...
class rml_parse(object):
    ...
    def _get_lang_dict(self):
        ...
        lang = self.localcontext.get('lang', tools.config.defaultLang) or tools.config.defaultLang
        lang_ids = pool_lang.search(self.cr,self.uid,[('code','=',lang)])
        if not lang_ids:
            lang_ids = pool_lang.search(self.cr,self.uid,[('code','=',tools.config.defaultLang)])
